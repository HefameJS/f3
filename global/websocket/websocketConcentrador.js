...
class ServidorWebSocketConcentrador {
	...
	constructor(puerto) {
		...
		this.#servidorHttp.listen(this.#puerto, () => {
			this.#log.info(`Servidor WebSocket Concentrador a la escucha en el puerto ${this.#puerto}`);
		});
		...
		this.#servidorWs.on('request', (request) => {
			...
			this.#log.info('Petición entrante')

			let idCliente = this.#clientes.findIndex(e => e === null);
			if (idCliente === -1) idCliente = this.#clientes.length;

			this.#log.info(`Se asigna el ID de conexión ${idCliente} a la conexión entrante`)

			let cliente = new ConexionConcentrador(idCliente, this, request);
...
module.exports = ServidorWebSocketConcentrador;