...
const retransmit = function (myId, retransmitId, force, cb) {
		...
		Imongo.findTxById(myId, retransmitId, function(err, dbTx) {
			...
			L.xi(myId, ['Se recupera la transmisión de la base de datos', dbTx]);
			...
			if (dbTx) {
				...
				if (dbTx.type !== txTypes.CREAR_PEDIDO) {
					L.xw(myId, ['No se retransmite la petición porque no es de tipo CREAR PEDIDO', dbTx.type]);
				...
				switch (dbTx.status) {
					...
					case txStatus.RECHAZADO_SAP:
						if (force) break;
						L.xw(myId, ['No se retransmite la petición porque no está en error y no se está forzando', dbTx.status]);
					...
					case txStatus.SISTEMA_SAP_NO_DEFINIDO:
						L.xd(myId, ['El estado de la transmisión es válido y será retransmitida a SAP', dbTx.status]);
						break;
					default:
						L.xw(myId, ['El estado de la transmisión no admite retransmitirla', dbTx.status]);
				...
				try {
					...
					L.xe(myId, ['El contenido de la transmisión no es correcto', error]);
				...
				L.xd(myId, ['El contenido de la transmisión es un pedido correcto, continuamos con la retransmisión', pedido]);
				...
				Isap.realizarPedido( myId, pedido, function(sapErr, sapRes, sapBody, abort) {
					if (sapErr) {
						if (abort) {
							...
							L.xe(myId, ['Incidencia en la comunicación con SAP', sapErr]);
