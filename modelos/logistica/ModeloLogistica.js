...
class Logistica {
	...
	limpiarEntrada(txId) {
		...
		if (this.incidencias && this.incidencias.concat) {
			this.incidencias.concat(incidenciasCabecera.getErrores());
		} else {
			this.incidencias = incidenciasCabecera.getErrores();
		}
		...
		if (this.lineas && this.lineas.forEach) {
			this.lineas.forEach((lineaLogistica) => {
				...
				if (incidenciasLinea.tieneErrores()) {
					if (lineaLogistica.incidencias && lineaLogistica.incidencias.concat) {
						lineaLogistica.incidencias.concat(incidenciasLinea.getErrores());
					} else {
						lineaLogistica.incidencias = incidenciasLinea.getErrores();
					}
				}
			});
		}
